...
function setAdmin(name, password, email) {
  const query = `
    INSERT INTO ${config.mysqlAdmin} (name, password , email) 
    VALUES ?`;
...
function grant(idUser, previlege_type, value) {
  const query = `
        UPDATE privilege SET is_grantable = ? WHERE idUser = ? AND privilege_type = ?`;

  const data = [[value], [idUser], [previlege_type]];
  return new Promise((resolve, reject) => {
    db.query(query, data, (err, rows) => {
      if (err) return reject(err);
      resolve(rows);
